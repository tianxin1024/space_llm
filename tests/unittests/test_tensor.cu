...
TEST(TensorMapTest, InsertDoesNotAllowDuplicateKey) {
    int *v1 = new int[4]{1, 10, 20, 30};
    Tensor t1 = Tensor{MEMORY_CPU, TYPE_INT32, {4}, v1};
...
TEST(TensorMapTest, GetValCorrectness) {
    ...
    TensorMap map({{"t1", t1}});
    EXPECT_TRUE(map.size() == 1);
    // throw excpetion since the map doesn't have a key "t3".
    ...
    EXPECT_TRUE(map.getVal<int>("t3", 3) == 3);
...
TYPED_TEST(TensorFuncTest, SliceCorrectness) {
    ...
    // std::cout << "t2: " << t2.getVal<T>() << std::endl;
    // t2.print<T>();
