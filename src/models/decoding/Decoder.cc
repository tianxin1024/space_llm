...
void Decoder<T>::forward(std::vector<Tensor> *output_tensors,
    ...
    QK_CHECK(input_tensors->size() == 7);
    QK_CHECK(output_tensors->size() == 5);
    ...
    for (uint l = 0; l < num_layer_; ++l) {
        ...
        invokeGeneralLayerNorm(decoder_normed_input_,
                               decoder_input,
                               decoder_layer_weights->at(l).pre_layernorm_weights.gamma,
                               decoder_layer_weights->at(l).pre_layernorm_weights.beta,
                               layernorm_eps_,
                               batch_size,
                               hidden_units_,
                               (float *)nullptr,
        ...
        self_attention_layer_->forward(&self_attention_output_tensors,
                                       ...
                                       &decoder_layer_weights->at(l).self_attention_weights);
        ...
        invokeGeneralAddBiasResidualPreLayerNorm(
            ...
            self_attn_output_,
            decoder_input,
            decoder_layer_weights->at(l).self_attn_layernorm_weights.gamma,
            decoder_layer_weights->at(l).self_attn_layernorm_weights.beta,
            decoder_layer_weights->at(l).self_attention_weights.attention_output_weight.bias,
            layernorm_eps_,
            batch_size,
            hidden_units_,
            (float *)nullptr,
        ...
        TensorMap cross_attention_output_tensors{
            ...
            {"key_cache", Tensor{MEMORY_GPU, data_type,
            ...
            {"value_cache", Tensor{MEMORY_GPU, data_type,
