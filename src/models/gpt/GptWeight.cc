...
template <typename T>
GptWeight<T>::GptWeight(const int hidden_units,
    ...
    QK_CHECK(num_layer_ % layer_para_size_ == 0);
    ...
    for (int l = 0; l < num_layer_; ++l) {
        if (isValidLayerParallelId(l)) {
            decoder_layer_weights.push_back(new GptDecoderLayerWeight<T>(
...
void GptWeight<T>::loadModel(std::string dir_path) {
    QKCudaDataType model_file_type = getModelFileType(dir_path + "/config.ini", "gpt");
...
template <typename T>
bool GptWeight<T>::isValidLayerParallelId(int l) {
