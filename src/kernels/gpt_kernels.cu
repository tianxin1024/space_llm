...
void invokeInputIdsEmbeddingLookupPosEncoding(T *from_tensor,
                                              ...
                                              cudaStream_t stream) {
    dim3 grid(std::min(batch_size * length, 65536));
    dim3 block(std::min(hidden_units, 512));
    const bool has_output_ids = output_ids != nullptr;
    QK_CHECK(!(has_output_ids && input_ids == nullptr));
...
void invokeInputIdsEmbeddingLookupPosEncodingSoftPrompt(inputIdsEmbeddingLookupPosEncodingSoftPromptParam<T> param) {
    dim3 grid(std::min(param.batch_size * param.beam_width * (param.max_input_length + param.max_prefix_soft_prompt_length),
                       65536));
    dim3 block(std::min(param.hidden_units, 512));
...
void invokeLookupHiddenStateOfLastToken(T *from_tensor,
    ...
    dim3 grid(std::min(grid_size, 65536));
...
void invokeTileGptPromptInputs(int *tiled_input_ids,
    ...
    dim3 block(std::min(1024, max_input_length));
...
void invokeCompactInputs(T *compact_input,
    ...
    const size_t elems_n = compact_size * seq_len * std::max(hidden_dimension, seq_len);
