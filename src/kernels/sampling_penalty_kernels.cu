...
void invokeApplyTemperaturePenalty(T *logits,
                                   ...
                                   const int vocab_size_padd,
                                   cudaStream_t stream) {
    dim3 block(std::min(vocab_size_padd, 1024));
    dim3 grid(std::min(static_cast<int>(batch_size * vocab_size_padd / block.x), 65536));
...
{
    const int block_size = std::min(batch_size, 1024);
