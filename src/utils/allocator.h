    ...
    Allocator(int device_id) :
        ...
        for (int i = 0; i < device_count; i++) {
            ...
            if (!peer_access_available) {
                QK_LOG_WARNING("Device " + std::to_string(device_id) + " peer access Device " + std::to_string(i)
                               + " is not available.");
                continue;
            }
    ...
    void free(void **ptr, bool is_host = false) const {
        ...
        if (*ptr != nullptr) {
            ...
            if (pointer_mapping_->count(address)) {
                ...
                QK_LOG_WARNING("pointer_mapping_ does not have information of ptr at %p.", address);
            }
        }
        *ptr = nullptr;
        return;
