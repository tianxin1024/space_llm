...
__global__ void cross_attention_kernel_opt(T *__restrict query_buf,
    ...
    int key_value_id = bid * (seq_len * head_num * size_per_head) + head_id * size_per_head; // TODO 这里是否是head_id
    ...
    // wach warp will have its own copy of sq
    ...
    for (int ite = warp_id; ite < length; ite += warp_num) {
        ...
        // For the first step, we should add bias to key memory cache.
        // The KV memory cache onlu need to be updated at the first step.
        if (step == 1) {
            for (int i = 0; i < elems_per_thread; ++i) {
                key_val_r.x[i] = (float)key_val_r.x[i] + (float)bias_r.x[i];
                if (do_ia3) {
                    key_val_r.x[i] = (float)key_val_r.x[i] * (float)(ia3_key_weights[(ia3_task * head_num + head_id) * size_per_head + tid]);
                }
            }
    ...
    for (int ite = warp_id; ite < length; ite += warp_num) {
        ...
        if (step == 1) {
            for (int i = 0; i < elems_per_thread; ++i) {
                key_val_r.x[i] = (float)key_val_r.x[i] + (float)bias_r.x[i];
                if (do_ia3) {
                    key_val_r.x[i] = (float)key_val_r.x[i] * (float)(ia3_value_weights[(ia3_task * head_num + head_id) * size_per_head + tid]);
                }
            }
